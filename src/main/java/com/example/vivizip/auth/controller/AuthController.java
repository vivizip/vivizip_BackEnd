...
public class AuthController {
    ...
    @PostMapping("/reissue")
    public ResponseEntity<ApiResponseDto<JwtToken>> reissueToken(@RequestBody @Valid TokenRequest request) {
        JwtToken newTokens = authService.issueTokens(request.refreshToken());
        log.info("토큰 재발급 완료");
        return ResponseEntity.ok(ApiResponseDto.onSuccess(newTokens));
    }

    @Operation(summary = "로그아웃", description = "refreshToken을 Redis에서 삭제하여 로그아웃 처리")
    @PostMapping("/logout")
    public ResponseEntity<ApiResponseDto<Void>> logout(@RequestBody @Valid TokenRequest request) {
